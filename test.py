...
#     model_path = os.path.join(args.model_path, f'sub-dataset{num_sub}', 'checkpoint.pth')
#     resize_dim = model_config[args.model_path]["embed_dim"] if args.distillation_token else None
#     model = create_model(
#         args.model,
#         pretrained=False,
#         num_classes=25,
...
#     teacher_model = get_models(args, division_num_classes, args.start_division, logger)
...
#     if args.eval:
#         test_stats = evaluate(data_loader_val, teacher_model, args.device)
...
# #                  embed_dim=128, num_classes=10, num_heads_list=[2, 4, 6, 8, 8]):
# #         super().__init__()
...
# #             TransformerEncoderLayer(embed_dim, num_heads=h)
...
# #         x = torch.cat((cls_tokens, x), dim=1) + self.pos_embed  # add pos embed
...
# #     embed_dim=128,
# #     num_classes=10,
# #     num_heads_list=[2, 4, 8, 8, 8]  # 每层 head 数不同
# # )

# # dummy_input = torch.randn(8, 3, 32, 32)  # batch size 8
