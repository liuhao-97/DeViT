...
reduce_neck = []
for i in range(len(head_mask[0])):
    if head_mask[0][i] == 0:
        reduce_neck= reduce_neck + np.arange(i*64, (i+1)*64).tolist()
    # zero_indices = [i for i, val in enumerate(head_mask[i]) if val == 0]
    # reduce_neck.append(zero_indices)
print(len(reduce_neck))
new_dict  = {}
cnt = 1
...
num_heads = 6 # need to change according to which model is pruned, from deit_base to dedeit (half pruned it should be 12, from dedeit(6 head) to dedeit_tiny it should be 6)
head_dim = 64
