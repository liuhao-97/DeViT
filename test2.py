...
def rebuild_small_model_from_pruned(large_model: nn.Module, neuron_mask: list, head_mask: list, num_classes: int) -> nn.Module:
    """
...
def debug_layer_outputs(large_model, small_model, neuron_mask, head_mask, sample_input):
    """
    逐层比较大模型 应用mask后 和小模型的输出,以调试权重迁移问题。
    ...
    Args:
        ...
        neuron_mask: MLP神经元掩码。
        head_mask: 注意力头掩码。
        sample_input: 一个批次的输入数据,例如 `next(iter(data_loader_val))[0]`。
    """
    print("🕵️  Starting layer-by-layer output comparison...")
    ...
    neuron_indices = torch.where(neuron_mask[0].cpu() == 1)[0]
    large_dim = large_model.embed_dim
    head_dim = large_dim // large_model.blocks[0].attn.num_heads
    embed_dim_indices = torch.cat(
        [torch.arange(idx * head_dim, (idx + 1) * head_dim) for idx in head_indices]
    ).long()
    ...
    embed_dim_indices = embed_dim_indices.to(device)
    neuron_indices = neuron_indices.to(device)

    def compare_tensors(name, large_tensor, small_tensor):
        # 手动对大模型的输出进行切片,以匹配小模型的维度
        if large_tensor.shape != small_tensor.shape:
             # 假设差异只在最后一个维度 (embedding dim)
            if large_tensor.dim() == 3 and large_tensor.shape[-1] != small_tensor.shape[-1]:
                 large_tensor_sliced = large_tensor.index_select(-1, embed_dim_indices)
            else:
                print(f"Cannot compare '{name}': Shape mismatch is not on the last dimension.")
                return False
        else:
            large_tensor_sliced = large_tensor

        match = torch.allclose(large_tensor_sliced, small_tensor, atol=1e5) # atol是绝对容忍度 atol=1e-5
        if not match:
            mse = F.mse_loss(large_tensor_sliced, small_tensor).item()
            print(f"❌ MISMATCH at '{name}'. MSE: {mse:.6e}")
        else:
            mse = F.mse_loss(large_tensor_sliced, small_tensor).item()
            print(f"✅ MATCH at '{name}'. MSE: {mse:.6e}")
        return match
    ...
    with torch.no_grad():
        ...
        # 大模型
        x_large = large_model.patch_embed(sample_input)
        x_large = torch.cat((large_model.cls_token.expand(x_large.shape[0], -1, -1),
                             large_model.dist_token.expand(x_large.shape[0], -1, -1), x_large), dim=1)
        x_large = large_model.pos_drop(x_large + large_model.pos_embed)
        
        # 小模型
        x_small = small_model.patch_embed(sample_input)
        x_small = torch.cat((small_model.cls_token.expand(x_small.shape[0], -1, -1),
                             small_model.dist_token.expand(x_small.shape[0], -1, -1), x_small), dim=1)
        x_small = small_model.pos_drop(x_small + small_model.pos_embed)

        if not compare_tensors("Initial Embedding", x_large, x_small): return
        ...
        for i in range(len(small_model.blocks)):
            ...
            # --- Attention Path ---
            large_norm1_out = large_block.norm1(x_large)
            small_norm1_out = small_block.norm1(x_small)
            if not compare_tensors(f"Block {i} - Norm1", large_norm1_out, small_norm1_out): return

            large_attn_out = large_block.attn(large_norm1_out)['output']
            small_attn_out = small_block.attn(small_norm1_out)['output']
            if not compare_tensors(f"Block {i} - Attention Output", large_attn_out, small_attn_out): return

            x_large_after_attn = x_large + large_block.drop_path(large_attn_out)
            x_small_after_attn = x_small + small_block.drop_path(small_attn_out)
            if not compare_tensors(f"Block {i} - After Attention Residual", x_large_after_attn, x_small_after_attn): return

            # --- MLP Path ---
            large_norm2_out = large_block.norm2(x_large_after_attn)
            small_norm2_out = small_block.norm2(x_small_after_attn)
            if not compare_tensors(f"Block {i} - Norm2", large_norm2_out, small_norm2_out): return

            large_mlp_out = large_block.mlp(large_norm2_out)
            small_mlp_out = small_block.mlp(small_norm2_out)
            if not compare_tensors(f"Block {i} - MLP Output", large_mlp_out, small_mlp_out): return

            # 更新下一轮的输入
            x_large = x_large_after_attn + large_block.drop_path(large_mlp_out)
            x_small = x_small_after_attn + small_block.drop_path(small_mlp_out)
            if not compare_tensors(f"Block {i} - Final Output", x_large, x_small): return
        
        # 3. Final Layers
        print("\n--- Comparing Final Layers ---")
        large_final_norm = large_model.norm(x_large)
        small_final_norm = small_model.norm(x_small)
        if not compare_tensors("Final Norm", large_final_norm, small_final_norm): return

        large_head_input = large_final_norm[:, 0]
        small_head_input = small_final_norm[:, 0]
        if not compare_tensors("Input to Head (CLS Token)", large_head_input, small_head_input): return
        
        large_logits = large_model.head(large_head_input)
        small_logits = small_model.head(small_head_input)
        # 最终的logits维度应该一致,不需要切片
        compare_tensors("Final Logits", large_logits, small_logits)
...
def main(args):
    ...
    mlp_neuron_shrink(model_to_retest, neuron_mask)
    attn_head_shrink(model_to_retest, head_mask)
    # test_stats = evaluate(data_loader=data_loader_val, model=model_to_retest, device=args.device)
    # print(test_stats)


    small_model = rebuild_small_model_from_pruned(
        large_model=model_to_retest,
    ...
    small_model.to(args.device) # 别忘了把小模型放回GPU

    # # 从验证数据加载器中取一个批次的数据作为输入
    sample_images, _ = next(iter(data_loader_val))
    ...
    # # --- 调试代码结束 ---
    # 验证重建后的小模型
    print("\nEvaluating the rebuilt small model...")
    test_stats = evaluate(data_loader=data_loader_val, model=small_model, device=args.device)
    print(f"Evaluation results for the small model: {test_stats}")
